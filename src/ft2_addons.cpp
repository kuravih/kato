...
kato::FreeTypeFont::FreeTypeFont(const char *_pcf, uint _size)
{
...
kato::FreeTypeFont::~FreeTypeFont()
{
    if (face)
        FT_Done_Face(face);
    if (library)
...
void kato::FreeTypeFont::drawGlyph(uint16_t *image, uint imgWidth, uint imgHeight, int x, int y, uint16_t intensity)
{
...
void kato::FreeTypeFont::renderText(uint16_t *image, uint imgWidth, uint imgHeight, const std::string &text, int startX, int startY, uint16_t intensity)
{
    int penX = startX;
    int penY = startY;
...
void kato::function::overlayInfo(kato::FreeTypeFont &font, uint16_t *buffer, const long width, const long height, const std::string &timestamp, const long exposureTime_ms, const float gain, const float framerate, const float temperature_C, const long br_x, const long br_y, const long tl_x, const long tl_y, const long frame_count, const long event_count)
{
